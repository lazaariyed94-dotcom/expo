...
- [iOS] Fix a data race on `EXTaskService`'s unsynchronized collections that could corrupt them and crash the app when one app context registered its task manager while another tore down, such as on a dev-client reload or `Updates.reloadAsync()`. ([#49558](https://github.com/expo/expo/pull/49558) by [@vonovak](https://github.com/vonovak))

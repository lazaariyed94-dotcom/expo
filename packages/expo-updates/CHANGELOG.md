...
- [iOS] Fix a data race on the updates event manager's observer, and stop an outgoing app context's teardown from detaching the observer the incoming context just installed, which silenced `Updates.useUpdates()` state events after a reload. ([#49561](https://github.com/expo/expo/pull/49561) by [@vonovak](https://github.com/vonovak))

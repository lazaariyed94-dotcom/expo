...
import { listFilesInDirectories, scanFilesRecursively } from '../utils';
...
async function scanInternalConfigs(
  ...
  const configPaths = await listFilesInDirectories(
    packagesDir,
    (name) => name === 'spm.config.json'
  );
  for (const configPath of configPaths) {
    ...
    const config = readJsonFile(path.join(packagesDir, configPath));
    if (!config) {
      continue;
    }
